/**
 * dsh-expression — 表情包管理面板(Client 半边)。
 *
 * 注册到设置页 `settings.section`(id: memes,标签「表情包」):
 *   - 搜索/分类过滤 + 缩略图网格(经 /dsh-memes 路由)
 *   - 上传新表情包(选文件 + 分类,base64 走 host.call → Host 写库)
 *   - 行内编辑(tag / caption / keywords)
 *   - 删除(确认后删索引 + 文件)
 * 数据经 Package-private RPC(host.call)与 Host 半边互通。
 */
'use strict'

// 包级样式(插件卸载自动清理);颜色走主题 token,自动适配深浅色。
const CSS = [
...
function MemePanel() {
  ...
  const load = async (query, tagf) => {
    setBusy(true)
    try {
      const res = await host.call('memes-list', { q: query || '', tag: tagf || '' })
      if (res && res.ok) {
        setMemes(res.memes)
        setTags(res.tags)
        setTotal(res.total)
  ...
  const onPickFile = (ev) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const res = await host.call('memes-upload', { tag: upTag.trim().toLowerCase(), fileName: file.name, dataBase64: data })
        setNotice(res && res.ok && res.meme ? '已上传: ' + res.meme.path : '上传失败')
  ...
  const onDelete = async (m) => {
    ...
    try {
      const res = await host.call('memes-delete', { path: m.path })
      setNotice(res && res.ok ? '已删除' : '删除失败')
      await load(q, tagFilter)
    } catch (error) {
